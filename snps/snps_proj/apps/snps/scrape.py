...
def genotype(rsid):
  ...
  soup = BeautifulSoup(urllib.urlopen('http://snpedia.com/index.php/Special:Browse/' + rsid, "").read(), "html.parser")
...
def snp(rsid, pair):
  ...
  soup = BeautifulSoup(urllib.request.urlopen('http://snpedia.com/index.php/Special:Browse/' + rsid + formatPair).read(), "html.parser")
  trows = soup('table')[1].find_all('tr')
  if len(trows) < 2:
